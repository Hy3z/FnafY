	...
	public void gameTick(GameTickEvent e) {
		if(isAtDoor) {
			...
			if(currentDelay==TICKDELAYCROSSROOM) {
				...
				Bukkit.getPluginManager().callEvent(new AnimatronicEnterRoomEvent(main.getRoomManager().getRoom(currentRoom), anim));
				List<ASAnimation> animations = main.getRoomManager().getRoom(currentRoom).getAnimations(anim);
